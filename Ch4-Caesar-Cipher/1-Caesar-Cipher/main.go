...
// since the alphabet is finite,
// we need to use the modulo operator % to "wrap around" the index
// if it exceeds the length of the alphabet.
func getOffsetChar(c rune, offset int) string {
	const alphabet = "abcdefghijklmnopqrstuvwxyz"
...
For example, in English, the letter e is by far the most commonly used.
If the attacker can find the most common letter in the ciphertext, then that letter very likely maps to the plaintext e.
Because a Caesar cipher uses a fixed-length shift, the key is immediately discovered and the ciphertext can be compromised.
*/
